...
private var levelBoundary : LevelBoundary;
...
function Awake () {
	...
	levelBoundary = LevelBoundary.GetInstance ();
	levelBounds = levelBoundary.bounds;
...
function SetTarget (newTarget : Transform, snap : boolean) {
	...
	if  (target) {
		...
		var targetRigidbody : Rigidbody = target.GetComponent (Rigidbody) as Rigidbody;
		if  (targetRigidbody)
...
function GetGoalPosition () {
	...
	var cameraTargetAttributes : CameraTargetAttributes = target.GetComponent (CameraTargetAttributes) as CameraTargetAttributes;
	...
	var goalPosition = target.position + Vector3 (widthOffset, heightOffset, -distance * distanceModifier);
	...
	var targetPlatformerController : PlatformerController = target.GetComponent (PlatformerController) as PlatformerController;
