...
private var areEmittersOn : boolean = false;
...
function ApplyGravity () {
	// Apply gravity
	var jumpButton : boolean = false;
	/*if (Input.touchCount > 0 )
	{
		for (var i : int = 0; i < Input.touchCount; ++i)
		{
			if (Input.GetTouch(i).position.y > 300)
			{
				jumpButton = true;
			}
		}
...
function UpdateEffects () {
	var wereEmittersOn : boolean = areEmittersOn;
	...
	if (wereEmittersOn != areEmittersOn) {
		...
		var tempList : ParticleEmitter[];
		tempList = GetComponentsInChildren (ParticleEmitter) as ParticleEmitter[];
	
		for (var emitter : ParticleEmitter in tempList ) {
...
function Update () {
	/*if (Input.touchCount > 0 )
	{
		...
		{
			if (Input.GetTouch(i).position.y > 300 && canControl) 
			{
