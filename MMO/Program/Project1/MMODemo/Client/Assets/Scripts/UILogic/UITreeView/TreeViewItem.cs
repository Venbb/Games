...
public class TreeViewItem : MonoBehaviour
{
    ...
    public int index = 0;
    public eOperationPart eOperation;
    ...
    public virtual void InitItemData(TreeViewData viewData)
    {
        index = viewData.index;
        eOperation = viewData.eOperation;
        ePart = viewData.ePart;
    }
    /// <summary>
    /// 点击上下文菜单按钮，元素的子元素改变显示状态
    /// </summary>
    public virtual void ContextButtonClick()
    {
        //上一轮刷新还未结束
        if (_isRefreshing)
        {
    ...
    void ChangeChildren(TreeViewItem tvi, bool value)
    {
        for (int i = 0; i < tvi.GetChildrenNumber(); i++)
        {
            if (value && i == 0)
            {
                Controler.ItemCheck(tvi.GetChildrenByIndex(i));
            }
