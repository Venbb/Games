...
public class TreeViewControl : MonoBehaviour
{
    ...
    /// 树形菜单中子元素的高度
    /// </summary>
    public int ChildItemHeight = 35;
    /// <summary>
    /// 所有子元素的鼠标点击回调事件
    /// </summary>
    public delegate void ClickItemdelegate(TreeViewItem item);
    ...
    public void ItemCheck(TreeViewItem treeView)
    {
        for (int i = 0; i < _treeViewItems.Count; i++)
        {
            if (_treeViewItems[i].GetComponent<TreeViewItem>().index == treeView.index)
            {
                _treeViewItems[i].GetComponent<UIPartEditor>().mSelectImage2.SetActive(true);
                if (ClickItemEvent != null)
                {
                    ClickItemEvent(_treeViewItems[i].GetComponent<TreeViewItem>());
                }
            }
            else
            {
                _treeViewItems[i].GetComponent<UIPartEditor>().mSelectImage2.SetActive(false);
            }
        }
    }

    public void OnClickExpanding(TreeViewItem treeView)
    {
        for (int i = 0; i < _treeViewItems.Count; i++)
        {
            if (_treeViewItems[i].GetComponent<TreeViewItem>().index == treeView.index)
            {
                _treeViewItems[i].GetComponent<UIPartEditor>().ContextButtonClick();
            }
            else
            {
                if (_treeViewItems[i].GetComponent<UIPartEditor>().IsExpanding)
                {
                    _treeViewItems[i].GetComponent<UIPartEditor>().ContextButtonClick();
                }
            }
    ...
    public void GenerateTreeView()
    {
        ...
        for (int i = 0; i < Data.Count; i++)
        {
            ...
            TreeViewItem tvit = item.GetComponent<TreeViewItem>();
            ...
            else
            {
                ...
                tvit.SetHierarchy(tvi.GetHierarchy() + 1);
                tvit.SetParent(tvi);
                tvi.AddChildren(tvit);
                item.SetActive(false);
            }

            item.transform.name = "TreeViewItem";
            tvit.InitItemData(Data[i]);
    ...
    void RefreshTreeViewChild(TreeViewItem tvi)
    {
        for (int i = 0; i < tvi.GetChildrenNumber(); i++)
        {
            ...
            _yIndex += (-(ChildItemHeight + VerticalItemSpace));
