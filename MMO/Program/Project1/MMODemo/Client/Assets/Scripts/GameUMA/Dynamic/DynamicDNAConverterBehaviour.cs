...
namespace UMA.CharacterSystem
{
    ...
    public class DynamicDNAConverterBehaviour : DynamicDNAConverterBehaviourBase
    {
        ...
        public void ResetAllStartingValues()
        {
            for (int i = 0; i < skeletonModifiers.Count; i++)
            {
                ...
                if (skeletonModifiers[i].property == SkeletonModifier.SkeletonPropType.Position || skeletonModifiers[i].property == SkeletonModifier.SkeletonPropType.Rotation)
                {
                    skeletonModifiers[i].ValueX.Set(0, thisValueX.y, thisValueX.z);
                    skeletonModifiers[i].ValueY.Set(0, thisValueY.y, thisValueY.z);
                    skeletonModifiers[i].ValueZ.Set(0, thisValueZ.y, thisValueZ.z);
                }
                else if (skeletonModifiers[i].property == SkeletonModifier.SkeletonPropType.Scale)
                {
                    skeletonModifiers[i].ValueX.Set(1, thisValueX.y, thisValueX.z);
                    skeletonModifiers[i].ValueY.Set(1, thisValueY.y, thisValueY.z);
                    skeletonModifiers[i].ValueZ.Set(1, thisValueZ.y, thisValueZ.z);
                }
            }
        }
        ...
        public void UpdateDynamicUMADnaBones(UMAData umaData, UMASkeleton skeleton, bool asReset = false)
        {
            ...
            if (startingPose != null && asReset == true)
            {
                for (int i = 0; i < startingPose.poses.Length; i++)
                {
					skeleton.Set(startingPose.poses[i].hash, startingPose.poses[i].position, startingPose.poses[i].scale, startingPose.poses[i].rotation);
				}
            }
