...
public class EnemyMove2R : MonoBehaviour {
    ...
    void Update()
    {
        if (script.isPause == isSetVictory)
        {
            isSetVictory = false;
            enemyState = 10;
            animationTest.state.SetAnimation(10, "victory_L", true);
        }
        switch (enemyState)
        {
            case 0:
                if (animationTest.AnimationName == "move_L")
                {
                    ...
                    if (timer > moveTime)
                    {
                        ...
                        if (attackTime == 0)
                        {
                            animationTest.AnimationName = "trigger_L";
                            enemyState = 3;
                        }
                        else animationTest.AnimationName = "stand_L";
                        //animationTest.loop = !animationTest.loop;
                    }
                    else
                    {
                ...
                else if (animationTest.AnimationName == "enter_L")
                {
                    timer += Time.deltaTime;
    ...
    void move()
    {
        if (attackTime < 0)
        {
            animationTest.state.SetAnimation(0, "attack_L", false);
            moveUnit = new Vector2(3f * moveDistance / attackMoveTime, -0.1f * moveDistance / attackMoveTime);
            enemyState = 100;
        }
        else
            ...
            {
                case 0:
                    animationTest.AnimationName = "move_L";
                    animationTest.loop = !animationTest.loop;
                    timer = 0;
                    animationState = 1;
                    if (attackTime == 1)
                    {
                        moveUnit = new Vector2(1.8f * moveDistance / moveTime, 0.1f * moveDistance / moveTime);
                        scaleRatio = 9f / 82f;
                    }
                    else
                    {
                        moveUnit = new Vector2(1.7f * moveDistance / moveTime, 0.3f * moveDistance / moveTime);
                        scaleRatio = 9f / 91f;
                    }
                    break;
                case 1:
                    animationTest.AnimationName = "stand_L";
                    animationTest.loop = !animationTest.loop;
                    animationState = 0;
                    break;
                default: break;
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        {
            animationTest.AnimationName = "dead_skill_L";
        ...
        {
            animationTest.AnimationName = "dead_normal_L";
            enemyState = 1;
